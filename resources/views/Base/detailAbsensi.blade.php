        ...
        <div class="row">
            <div class="col-md-12">
                <div class="callout">
                    ...
                    <small>Tanggal Kegiatan: {{ $kegiatan->tanggal_kegiatan }} sampai {{ $kegiatan->tanggal_selesai }}</small><br>
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <form action="/update-absensi" method="POST">
                        @csrf
                        @method('PUT')
                            ...
                            <tbody class="text-left">
                                ...
                                <tr>
                                    ...
                                    <td class="pr-3 col-md-1">
                                        <input type="text" value="{{ $item->keterangan }}" name="keterangan[{{ $item->id }}]" class="form-control keterangan-input text-center" style="{{ ($item->absensi === 'Izin') ? 'display: block;' : 'display: none;' }}">
                                    </td>
                                    <td class="pr-3 col-md-2">
                                        <div class="form-group">
                                            <select class="form-control select-absensi" name="absensi[{{ $item->id }}]">
                                                <option value="Hadir"{{ ($item->absensi === 'Hadir') ? 'selected' : '' }}>Hadir</option>
                                                <option value="Alpa"{{ ($item->absensi === 'Alpa') ? 'selected' : '' }}>Alpa</option>
                                                <option value="Izin"{{ ($item->absensi === 'Izin') ? 'selected' : '' }}>Izin</option>
                                            </select>
                                        </div>
                                    </td>
                                </tr>
                        ...
                        <button type="submit" class="mt-3 mb-3 btn btn-primary">Update Absensi</button>
                    </form>
                </div>
            </div>
        </div>
